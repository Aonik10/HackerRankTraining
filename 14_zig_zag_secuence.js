...
function processData(input) {
    ...
    let [t, n, arrayStr] = input.split("\n");
    let array = arrayStr.split(" ");
    console.log(findZigZagSequence(array));
}

function findZigZagSequence(array) {
    let k = array.length / 2;
    let sortedArray = array.sort((a, b) => a - b);
    let zig = sortedArray.slice(0, k);
    let zag = sortedArray.slice(k).reverse();
    return zig.join(" ") + " " + zag.join(" ");
}
